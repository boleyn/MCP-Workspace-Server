...
class DirectoryTreeNode:
    ...
    def __init__(
        ...
        Args:
            path: Path this node represents (real filesystem path)
    ...
    def to_dict(self) -> Dict:
        ...
        Returns:
            Dictionary representing this node and its children (uses display_path)
        """
...
class AdvancedFileOperations:
    ...
    async def directory_tree(
        ...
        # Create root node with virtual path
        virtual_path = self.validator.real_to_virtual(abs_path)
        root_node = DirectoryTreeNode(abs_path, True, 0, display_path=virtual_path)
            ...
            for entry in entries:
                ...
                if pattern and not entry.match(pattern):
                    ...
                    try:
                        ...
                        # Create and add the child node with virtual path
                        virtual_path = self.validator.real_to_virtual(entry)
                        child = DirectoryTreeNode(
                            entry, is_dir, node.depth + 1, display_path=virtual_path
                        )
    ...
    async def find_duplicate_files(
        ...
        for size, files in size_groups.items():
            ...
            # Add duplicate groups to results (use virtual paths)
            for file_hash, hash_files in hash_groups.items():
                if len(hash_files) >= 2:
                    duplicates[file_hash] = [
                        self.validator.real_to_virtual(f) for f in hash_files
                    ]
    ...
    async def find_empty_directories(
        ...
        async def scan_for_empty_dirs(dir_path: Path) -> bool:
            ...
            try:
                ...
                if not entries:
                    # Found an empty directory - use virtual path
                    virtual_path = self.validator.real_to_virtual(dir_path)
                    empty_dirs.append(virtual_path)
                    return True
