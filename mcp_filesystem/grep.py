            ...
            async def scan_dir(dir_path: Path, current_depth: int = 0) -> None:
                ...
                try:
                    ...
                    for entry in entries:
                        try:
                            # Check if path is allowed using is_path_allowed (accepts Path objects)
                            if not self.validator.is_path_allowed(entry):
                                continue
        ...
        for i, file_path in enumerate(files_to_search):
            ...
            try:
                ...
                try:
                    ...
                    virtual_path = self.validator.real_to_virtual(file_path)
                    result.add_file_error(virtual_path, "Binary file")
                    continue
                        ...
                        while start_pos <= len(search_line) - len(search_pattern):
                            ...
                            if not whole_word or is_whole_word(
                                ...
                                if count_only:
                                    ...
                                    # Use virtual path for display
                                    virtual_path = self.validator.real_to_virtual(
                                        file_path
                                    )
                                    match_obj = GrepMatch(
                                        file_path=virtual_path,
                ...
                # Update file counts for count-only mode (use virtual path)
                if count_only and file_matches > 0:
                    virtual_path = self.validator.real_to_virtual(file_path)
                    result.file_counts[virtual_path] = file_matches
                ...
                virtual_path = self.validator.real_to_virtual(file_path)
                result.add_file_error(virtual_path, str(e))
                continue
            except Exception as e:
                virtual_path = self.validator.real_to_virtual(file_path)
                result.add_file_error(virtual_path, f"Error: {str(e)}")
                continue
