...
def format_timestamp(timestamp: float) -> str:
    """Convert Unix timestamp to ISO format string with UTC+8 timezone and seconds precision.

    Args:
        timestamp: Unix timestamp (seconds since epoch)

    Returns:
        ISO format string with UTC+8 timezone, e.g. "2025-12-04T14:16:02+08:00"
    """
    dt = datetime.fromtimestamp(timestamp, tz=UTC_PLUS_8)
...
class FileInfo:
    ...
    def __init__(self, path: Path, display_path: Optional[str] = None):
        ...
        Args:
            path: Path to the file or directory (real filesystem path)
            display_path: Optional virtual path to display instead of real path

        Raises:
            FileNotFoundError: If the file or directory does not exist
        """
        self.path = path
        self.display_path = display_path if display_path else str(path)
        self.stat = path.stat()
        self.is_dir = path.is_dir()
        self.is_file = path.is_file()
        self.is_symlink = path.is_symlink()
        self.size = self.stat.st_size
        self.created = format_timestamp(self.stat.st_ctime)
        self.modified = format_timestamp(self.stat.st_mtime)
        self.accessed = format_timestamp(self.stat.st_atime)
        self.name = path.name
    ...
    def to_dict(self) -> Dict:
        ...
        Returns:
            Dictionary with file information (uses display_path for path field)
        """
        return {
            "name": self.name,
    ...
    def __str__(self) -> str:
        ...
        Returns:
            Formatted string with file information (uses display_path)
        """
        ...
        return (
            f"{file_type}{symlink_info}: {self.display_path}\n"
            f"Size: {size_str}\n"
...
class FileOperations:
    ...
    async def read_file(self, path: str, encoding: str = "utf-8") -> str:
        ...
        Args:
            path: Virtual path string to the file (NOT a Path object)
        ...
        if not isinstance(path, str):
            raise TypeError(f"read_file expects str (virtual path), got {type(path).__name__}")
    ...
    async def read_file_binary(self, path: str) -> bytes:
        ...
        """
        if not isinstance(path, str):
            raise TypeError(f"read_file_binary expects str (virtual path), got {type(path).__name__}")
    ...
    async def write_file(
        ...
        """
        if not isinstance(path, str):
            raise TypeError(f"write_file expects str (virtual path), got {type(path).__name__}")
    ...
    async def list_directory(
        ...
        Args:
            path: Path to the directory (virtual path if virtual_root is set)
            include_hidden: Whether to include hidden files (starting with .)
            pattern: Optional glob pattern to filter files

        Returns:
            List of file/directory information dictionaries (with virtual paths)
        ...
        try:
            ...
            for entry in entries:
                ...
                try:
                    # Convert real path to virtual path for display
                    virtual_path = self.validator.real_to_virtual(entry)
                    info = FileInfo(entry, display_path=virtual_path)
    ...
    async def get_file_info(self, path: Union[str, Path]) -> FileInfo:
        ...
        Args:
            path: Path to the file or directory (virtual path if virtual_root is set)

        Returns:
            FileInfo object with detailed information (with virtual path)
        ...
        # Convert real path to virtual path for display
        virtual_path = self.validator.real_to_virtual(abs_path)
        return FileInfo(abs_path, display_path=virtual_path)
    ...
    async def read_file_lines(
        ...
        try:
            ...
            # Prepare metadata (use virtual path for display)
            virtual_path = self.validator.real_to_virtual(abs_path)
            metadata = {
                "path": virtual_path,
                "offset": offset,
    ...
    async def search_files(
        ...
        Args:
            root_path: Starting directory for search (virtual path if virtual_root is set)
        ...
        Returns:
            List of matching file information (with virtual paths)
        ...
        if content_match is None:
            for file_path in matching_files[:max_results]:
                try:
                    ...
                    # Convert real path to virtual path for display
                    virtual_path = self.validator.real_to_virtual(file_path)
                    info = FileInfo(file_path, display_path=virtual_path)
                    results.append(info.to_dict())
                except (PermissionError, FileNotFoundError):
                    # Skip files we can't access
                    pass
